...
lifecycle of event sources. It supports both HA mode (with leader
election) and non-HA mode.

The manager uses a two-phase initialization:
1. initialize() - Creates queues and prepares infrastructure (happens
   immediately)
2. start_sources() - Spawns source tasks (happens after becoming leader
   in HA mode)
...
from drools.ruleset import get_ha_stats
...
from ansible_rulebook.util import send_ha_stats
...
class SourceManager:
    ...
    Handles initialization, starting, and stopping of event sources with
    support for both HA and non-HA modes.

    Two-Phase Lifecycle:
    1. Initialize phase - Sets up queues, feedback queues, and infrastructure
    2. Execution phase - Starts/stops source tasks based on leadership
    ...
    Usage:
        ...
        # Phase 2a: Start sources (after becoming leader or immediately
        # in non-HA)
        await manager.start_sources()

        # Phase 2b: Stop sources (when losing leadership)
    ...
    def initialize(
        ...
        This allows the rule engine to start and send heartbeats even
        before sources are running (important for HA mode where we wait
        for leadership).
        ...
        Args:
            ...
            leader_election: Optional LeaderElection instance for HA mode
        ...
        # Store leader election instance for HA mode
        self._leader_election = leader_election

        logger.info("Initializing SourceManager (Phase 1)")
    ...
    async def start_sources(self) -> List[asyncio.Task]:
        ...
        In HA mode: This method will BLOCK waiting for leadership before
        starting sources. Only the leader will actually start sources.

        In non-HA mode: Sources start immediately.
        ...
        # In HA mode, we must wait for leadership before starting sources
        if self._leader_election is not None:
            logger.info(
                "HA mode: Waiting for rulesets to be ready before "
                "attempting leadership..."
            )
            await self._rulesets_ready_event.wait()
            logger.info("Rulesets ready - now waiting for leadership")

            # This BLOCKS until this worker becomes the leader
            # Only the leader proceeds past this point
            await self._leader_election.wait_for_leadership()
            logger.info("Became LEADER - now starting sources")

            # Enable leader mode in drools
            enable_leader()
            logger.info("Leader mode enabled in drools engine")
            if self._event_log:
                await send_ha_stats(self._event_log, get_ha_stats())
            else:
                print(get_ha_stats())
        else:
            logger.info("Non-HA mode: Starting sources immediately")
    ...
    async def _broadcast_shutdown(self, shutdown: Shutdown) -> None:
        ...
        """
        source_queues = list(self._source_queue_map.values())
        await broadcast(source_queues, shutdown)

    async def stop_sources(self, reason: str = "Shutdown requested") -> None:
        """
        Stop all running sources.

        This broadcasts a shutdown message to all sources and cancels their
        tasks. In HA mode, this should be called when losing leadership.
        ...
        # Broadcast shutdown to all sources
        try:
    ...
    def _get_feedback_queue(
        ...
        # Check if requires_feedback_queue
        if not source.source_args.get("requires_feedback_queue", False):
            raise SourcePluginFeedbackMisconfiguredException(
                source_name=source.name
            )

        # Get or create feedback queue
        if source.name not in source_feedback_queues:
            return asyncio.Queue()

        return source_feedback_queues[source.name]
    ...
    def signal_rulesets_ready(self) -> None:
        ...
        This allows start_sources() to proceed with enable_leader() when
        the system is fully ready to process events.
        """

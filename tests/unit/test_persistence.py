...
class TestEnablePersistence:
    ...
    def test_enable_persistence_standalone_mode(
        self, mock_lang, postgres_variables, reset_settings
    ):
        """Test enabling persistence without persistence_id."""
        args = argparse.Namespace()
        args.persistence_id = None
        args.id = "worker-1"

        persistence.enable_persistence(args, postgres_variables)

        assert settings.persistence_enabled is True
        call_kwargs = mock_lang.initialize_ha.call_args.kwargs
        assert call_kwargs["uuid"] == "standalone"
...
class TestGetConfigParams:
    ...
    def test_get_config_params_defaults(self):
        ...
        assert result["syncInterval"] == 1000
        assert result["stateCheckInterval"] == 500

...
WORKFLOW_TEMPLATE_ERRORS = [
    ...
    (
        "wf does not exist",
        WorkflowJobTemplateNotFoundException("wf does not exist"),
    ),
]
...
@pytest.mark.asyncio
async def test_run_workflow_template_exception(err_msg, err):
    queue = asyncio.Queue()
    metadata = Metadata(
        rule="r1",
        rule_set="rs1",
...
DROOLS_CALLS = [
    ...
    (
        "ansible_rulebook.action.run_workflow_template.lang.post",
...
@pytest.mark.asyncio
async def test_run_workflow_template_retries():
    queue = asyncio.Queue()
    metadata = Metadata(
        rule="r1",
        rule_set="rs1",

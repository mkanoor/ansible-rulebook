...
def setup_vars_file(temp_dir: Union[str, Path]) -> Path:
    ...
    vars_data["drools_db_file_path"] = str(
        temp_dir / "test.h2;AUTO_SERVER=TRUE;"
    )
...
async def terminate_process(
    ...
    # Add a delay to ensure H2 database file locks are fully released
    # H2 embedded database can hold file locks that take time to release
    # even after the process exits
    LOGGER.info("Waiting for database locks to be released...")
